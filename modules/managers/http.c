...
void print_request_details(struct mg_connection *conn) {
    ...
    if (req_info == NULL) {
        _utils_printf(NULL, "Erreur : Impossible de récupérer les infos de la requête.\n");
        return;
    }

    _utils_printf(NULL, "\n--- Détails de la Requête ---\n");
    _utils_printf(NULL, "Méthode       : %s\n", req_info->request_method);
    _utils_printf(NULL, "URI           : %s\n", req_info->local_uri); // ou req_info->uri selon version
    _utils_printf(NULL, "Version HTTP  : %s\n", req_info->http_version);
    _utils_printf(NULL, "IP Client     : %s\n", req_info->remote_addr);
    _utils_printf(NULL, "Port Client   : %d\n", req_info->remote_port);
    _utils_printf(NULL, "Port Serveur  : %d\n", req_info->server_port);
    
    if (req_info->query_string != NULL) {
        _utils_printf(NULL, "Query String  : %s\n", req_info->query_string);
    }
...
int send_http_response(struct mg_connection *conn, int code, 
    ...
    if (!is_chunked) {
      ...
      while (offset < content_length) {
        ...
        if (current_len < remaining) {
            ...
            if (current_len == 0) { _utils_printf(NULL, "\n\nStrange: Chunk current_len == 0\n\n"); }
        }
...
static int request_handler(struct mg_connection *conn, void *cb_data) {
    ...
    if (strcmp(req_info->local_uri, "/") == 0) {
      ...
      if (_utils_vedis_parse_value_isok(value)) {
        ...
        _utils_printf(NULL, "len: %d\n", strlen(value->_string_ptr));
...
void print_http_connections(struct mg_context *ctx) {
    ...
    while (mg_get_connection_info(ctx, idx, buffer, sizeof(buffer)) > 0) {
      //if (strstr(buffer, "\"state\" : \"not used\"") == NULL && strstr(buffer, "\"state\" : \"closed\"") == NULL) { // strstr(buffer, "\"closed\"") == NULL
        _utils_printf(NULL, "Connection [%d]: %s\n", idx, buffer);
      //}
      idx++;
    }
    _utils_printf(NULL, "Total: %d connections\n", idx);
    _utils_printf(NULL, "--------------------------\n");
}
#endif
#if defined(USE_SERVER_STATS)
void print_http_stats(struct mg_context *ctx) {
    char buffer[4096]; // Buffer large pour le JSON
    int len;
