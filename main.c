        ...
        if (linenoise_line != linenoiseEditMore) {
          ...
          if (linenoise_line) {
            ...
            if (is_command_exist) {
              ...
              if (linenoise_line) { if (strncmp(linenoise_line, "/test_vedis", strlen("/test_vedis")) == 0) { free(linenoise_line); linenoise_line = NULL; vedis_test(vedis_managers); } }
              #if defined(PLOUCKY_ENABLE_VEDIS_CMD)
                if (linenoise_line) { if (strncmp(linenoise_line, "/vedis ", strlen("/vedis ")) == 0) { _managers_vedis_exec_cmdcli(vedis_managers, linenoise_line); free(linenoise_line); linenoise_line = NULL; } }
              #endif
              #if defined(MG_EXPERIMENTAL_INTERFACES) && defined(USE_SERVER_STATS)
